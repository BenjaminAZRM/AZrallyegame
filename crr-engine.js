...
// completeDepuis (optionnel) = timestamp où toutes les spéciales ont été saisies.
// Un rallye complété reste "encours" pendant GRACE_TERMINE ms (24 h), puis "termine".
const GRACE_TERMINE = 24 * 3600 * 1000;
function etat(rallye, maintenant, completeDepuis) {
  ...
  const complet = courues >= nb && nb > 0;
  if (complet) {
    // période de grâce : visible en "en cours" pendant 24 h après la complétion
    if (completeDepuis && (maintenant - completeDepuis) < GRACE_TERMINE) return 'encours';
    return 'termine';
  }
